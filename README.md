...
## Correcciones v1.3.0 – v1.3.3 (tras las pruebas en servidor real)

| Síntoma en el servidor | Causa raíz (verificada en el código de Jellyfin 10.11.11) | Corrección |
|---|---|---|
| **v1.3.3** — **«Escanear e importar»** devuelve siempre `0, 0` aunque importe | El JS no parseaba el cuerpo JSON de las peticiones `POST` (`ApiClient.ajax` sin `dataType:'json'` devuelve un `Response`). Afectaba también a **«Validar ruta»**. | `req()` reescrito con `fetch` + `Authorization`, que **siempre lee y parsea** la respuesta. |
| **v1.3.3** — se **elimina «Duración (segundos)»** del editor de anuncios | Había dos números confusos. | La cuenta regresiva **ES** «Permitir omitir después de»: cuenta N → 0 y ahí se habilita *Omitir* (o el anuncio se cierra si no se permite omitir). Los anuncios nuevos toman ese valor de **«Tiempo predeterminado del anuncio»**. |
| **v1.3.3** — «Modo de anuncios» aún parecía invertido con datos antiguos | Anuncios de *Escanear* editados con versiones ≤ 1.3.1 perdieron su origen. | Además de conservarlo al editar (v1.3.2), **«Escanear»** ahora **re-clasifica** como automáticos los anuncios cuyo nombre sigue siendo el del archivo. Log de diagnóstico. |
...
| **v1.3.2** — la cuenta regresiva empezaba en **5** aunque la duración fuera **10** | El texto mostraba *segundos hasta poder omitir*, no el tiempo del anuncio. | Ver la fila de v1.3.3: ahora hay **un solo número** por anuncio. |
...
```powershell
pwsh ./build/package.ps1              # -> artifacts/jellyfin-startup-ads_1.3.3.0.zip
```
...
3. `git tag v1.3.3 && git push origin v1.3.3`
4. Crear el *GitHub Release* `v1.3.3` y subir el ZIP como asset.
...
```bash
sudo mkdir -p "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.3.0"
sudo unzip artifacts/jellyfin-startup-ads_1.3.3.0.zip \
     -d "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.3.0"
sudo chown -R jellyfin:jellyfin "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.3.0"
...
**Windows**: `%ProgramData%\Jellyfin\Server\plugins\Jellyfin Startup Ads_1.3.3.0\`
...
## Mecanismo de inyección en Jellyfin Web (v1.3.3)
...
capa de imagen (solo lectura). La inyección **en memoria** de v1.3.3 no toca esos
...
Plugins: `/config/plugins/Jellyfin Startup Ads_1.3.3.0/` (dentro del volumen
...
| Botones «Guardar anuncio» / «Cancelar» sin efecto (v1.2 y anteriores) | Corregido en **v1.3.3**. Actualiza el plugin. |
| El modal de anuncios sale a la derecha / recortado (v1.2 y anteriores) | Corregido en **v1.3.3** (el modal se traslada a `<body>`). |
| El plugin aparece con nombre raro en «Mis complementos» | Instala el **ZIP completo** (DLL + `meta.json`) en una carpeta `Jellyfin Startup Ads_1.3.3.0`, no solo el DLL. |
| El plugin no aparece en el **Catálogo** | Falta la *GitHub Release* `v1.3.3` con el ZIP subido como asset (el `sourceUrl` del manifest da 404 hasta entonces). |
...
| Docker: nada que hacer con `web/` | Correcto — v1.3.3 no toca `jellyfin-web` en disco. |
...
2. **Jellyfin core no tiene API oficial de inyección** (PR #9095 cerrado). El
   mecanismo de v1.3.3 usa `IStartupFilter` (extensión estándar de ASP.NET Core).
...
6. **Qué está probado y qué no** (ver también *Prueba real en el servidor*):
   ...
   - ✅ *Packaging*: ZIP reproducible (`8d0695de09187c312ea49e470a554987`, 48 240 B),
     solo `Jellyfin.Plugin.StartupAds.dll` + `meta.json` (verificado: sin
     `Jellyfin.Controller.dll` / `Jellyfin.Model.dll`).
   - ⚠️ *Análisis contra el código fuente de Jellyfin 10.11.11*: los bugs de v1.3.3 se
...
BUILD      pwsh ./build/package.ps1
           # -> artifacts/jellyfin-startup-ads_1.3.3.0.zip  (MD5 en release-info.json)

UPLOAD     scp artifacts/jellyfin-startup-ads_1.3.3.0.zip usuario@servidor:/tmp/

INSTALL    ssh usuario@servidor
           sudo mkdir -p "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.3.0"
           sudo unzip -o /tmp/jellyfin-startup-ads_1.3.3.0.zip \
                -d "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.3.0"
           sudo chown -R jellyfin:jellyfin "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.3.0"
