...
## Correcciones v1.3.0 – v1.3.2 (tras las pruebas en servidor real)
...
| **v1.3.2** — la cuenta regresiva empezaba en **5** aunque la duración fuera **10** | El texto mostraba *segundos hasta poder omitir* (`SkipAfterSeconds`, 5), no el tiempo restante del anuncio. | La cuenta ahora refleja la **duración del anuncio** (10 → 0); el botón *Omitir* se habilita al cumplirse `SkipAfterSeconds`. |
...
| **v1.3.2** — **«Modo de anuncios»** parecía invertido | Al **editar** un anuncio, el formulario no reenvía `AutoGenerated` → un anuncio de *Escanear* pasaba a *Manual* al guardarlo. (El filtro backend `Manual`↔creado a mano / `Automatic`↔escaneado es correcto, con test.) | `Update` **conserva** el `AutoGenerated` existente. Nueva columna **«Origen»** (Manual/Automático) en la tabla para verlo. |
| **v1.3.2** — el editor mostraba campos que no aplicaban al tipo | — | El editor muestra **solo los campos del tipo elegido** (Imagen/Vídeo/Texto/Multimedia) y **filtra el desplegable de archivos** por tipo. |
| **v1.3.2** — enlace poco visible | `GetPages()` no marcaba el menú principal. | `EnableInMainMenu = true` → enlace directo en el menú lateral. `Plugin.cs` vuelve al constructor estándar de 2 parámetros (más fiable al cargar). |
...
```powershell
pwsh ./build/package.ps1              # -> artifacts/jellyfin-startup-ads_1.3.2.0.zip
```
...
3. `git tag v1.3.2 && git push origin v1.3.2`
4. Crear el *GitHub Release* `v1.3.2` y subir el ZIP como asset.
...
```bash
sudo mkdir -p "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.2.0"
sudo unzip artifacts/jellyfin-startup-ads_1.3.2.0.zip \
     -d "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.2.0"
sudo chown -R jellyfin:jellyfin "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.2.0"
...
**Windows**: `%ProgramData%\Jellyfin\Server\plugins\Jellyfin Startup Ads_1.3.2.0\`
...
## Mecanismo de inyección en Jellyfin Web (v1.3.2)
...
capa de imagen (solo lectura). La inyección **en memoria** de v1.3.2 no toca esos
...
Plugins: `/config/plugins/Jellyfin Startup Ads_1.3.2.0/` (dentro del volumen
...
| Botones «Guardar anuncio» / «Cancelar» sin efecto (v1.2 y anteriores) | Corregido en **v1.3.2**. Actualiza el plugin. |
| El modal de anuncios sale a la derecha / recortado (v1.2 y anteriores) | Corregido en **v1.3.2** (el modal se traslada a `<body>`). |
| El plugin aparece con nombre raro en «Mis complementos» | Instala el **ZIP completo** (DLL + `meta.json`) en una carpeta `Jellyfin Startup Ads_1.3.2.0`, no solo el DLL. |
| El plugin no aparece en el **Catálogo** | Falta la *GitHub Release* `v1.3.2` con el ZIP subido como asset (el `sourceUrl` del manifest da 404 hasta entonces). |
...
| Docker: nada que hacer con `web/` | Correcto — v1.3.2 no toca `jellyfin-web` en disco. |
...
2. **Jellyfin core no tiene API oficial de inyección** (PR #9095 cerrado). El
   mecanismo de v1.3.2 usa `IStartupFilter` (extensión estándar de ASP.NET Core).
...
6. **Qué está probado y qué no** (ver también *Prueba real en el servidor*):
   ...
   - ✅ *Packaging*: ZIP reproducible (`6a5d822f8cbb21ca8a6abaa279f0de01`, 47 139 B),
     solo `Jellyfin.Plugin.StartupAds.dll` + `meta.json` (verificado: sin
     `Jellyfin.Controller.dll` / `Jellyfin.Model.dll`).
   - ⚠️ *Análisis contra el código fuente de Jellyfin 10.11.11*: los bugs de v1.3.2 se
...
BUILD      pwsh ./build/package.ps1
           # -> artifacts/jellyfin-startup-ads_1.3.2.0.zip  (MD5 en release-info.json)

UPLOAD     scp artifacts/jellyfin-startup-ads_1.3.2.0.zip usuario@servidor:/tmp/

INSTALL    ssh usuario@servidor
           sudo mkdir -p "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.2.0"
           sudo unzip -o /tmp/jellyfin-startup-ads_1.3.2.0.zip \
                -d "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.2.0"
           sudo chown -R jellyfin:jellyfin "/var/lib/jellyfin/plugins/Jellyfin Startup Ads_1.3.2.0"
